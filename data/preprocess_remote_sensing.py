...
def parse_npz_and_nc_data():
    ...
    vapor = np.reshape(scaler.fit_transform(np.reshape(vapor, (-1, 320*440))), (-1, 320, 440))
    cloud = np.reshape(scaler.fit_transform(np.reshape(cloud, (-1, 320*440))), (-1, 320, 440))
    rain = np.reshape(scaler.fit_transform(np.reshape(rain, (-1, 320*440))), (-1, 320, 440))

    data = []
    target = []
    for i in range(sst.shape[0] - params.sequence_length + 1 - params.lead_time):
        data.append({'sst': sst[i:i+params.sequence_length].astype(np.float32),
                     'uwind': uwind[i:i+params.sequence_length].astype(np.float32),
                     'vwind': vwind[i:i+params.sequence_length].astype(np.float32),
                     'vapor': vapor[i:i+params.sequence_length].astype(np.float32),
                     'cloud': cloud[i:i+params.sequence_length].astype(np.float32),
                     'rain': rain[i:i+params.sequence_length].astype(np.float32)})
...
def write_records(data, filename):
    ...
    for s, t in zip(series, target):
        example = tf.train.Example(features=tf.train.Features(feature={
            ...
            'vapor': _bytes_feature(s['vapor'].tobytes()),
            'cloud': _bytes_feature(s['cloud'].tobytes()),
            'rain': _bytes_feature(s['rain'].tobytes()),
